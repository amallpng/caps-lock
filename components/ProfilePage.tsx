...
const ProfilePage: React.FC<ProfilePageProps> = ({ user, onUserUpdate }) => {
  ...
  return (
    <div className="w-full max-w-5xl flex flex-col items-center gap-8">
      <div className="w-full flex flex-col md:flex-row items-center justify-center gap-8 p-6 bg-[#D3CFC2]/50 rounded-sm border-2 border-[#A9A391]">
        <div className="relative group">
          <Avatar avatarKey={user.profilePic} className="w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-[#4A6B69]" />
        ...
        <div className="text-center md:text-left">
          {isEditing ? (
            <div className="flex items-center gap-2">
              <input
                ...
                className="bg-[#F1EFE9] text-3xl font-bold text-[#282828] p-2 rounded-sm border border-[#A9A391] focus:outline-none focus:ring-2 focus:ring-[#4A6B69]"
              />
              <button onClick={handleUsernameChange} className="btn-vintage py-2 px-3 text-sm">Save</button>
              <button onClick={() => setIsEditing(false)} className="text-gray-500 hover:underline text-sm">Cancel</button>
            </div>
          ) : (
            <div className="flex items-center gap-4">
              <h1 className="text-4xl font-bold text-[#282828]">{user.username}</h1>
              <button onClick={() => setIsEditing(true)} aria-label="Edit username" className="text-gray-500 hover:text-[#282828]">
          ...
          <p className="text-lg text-gray-600 mt-2">Best WPM: <span className="font-bold text-[#4A6B69]">{user.bestWpm}</span></p>
        </div>
      </div>
      ...
      <div className="w-full">
        <h2 className="text-2xl font-bold text-[#4A6B69] mb-4 text-center">Earned Badges ({completedBadges.length}/{TASKS.length})</h2>
        {completedBadges.length > 0 ? (
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-4 bg-[#D3CFC2]/50 p-6 rounded-sm border-2 border-dashed border-[#A9A391]">
            {completedBadges.map(task => (
                <div key={task.id} className="flex flex-col items-center p-2 rounded-sm bg-[#F1EFE9] border border-[#A9A391] text-center">
                    <Badge task={task} />
                </div>
            ))}
          </div>
        ) : (
          <p className="text-center text-gray-500 py-8">No badges earned yet. Go to Challenge Mode to start collecting them!</p>
        )}
      </div>
    </div>
  );
